...
#pred_path = 'C:/MMA AI in Finance Course MAY 2024/LECTURES/WEEK1/predicted/linear_all_stock_exret_0.csv'
pred_path = 'C:/MMA AI in Finance Course MAY 2024/LECTURES/WEEK2/ipca_12_fits_pred.csv'
pred = pd.read_csv(pred_path, parse_dates=['date'])
...
monthly_port = pred_mve.groupby(['year', 'month', 'rank']).apply(lambda df:
                                pd.Series(np.average(df['real_exret'], weights=df['mve_m'], axis = 0)))
...
long_short_position['end_weight'] = long_short_position['start_weight'] * (1 + long_short_position['real_exret'])
