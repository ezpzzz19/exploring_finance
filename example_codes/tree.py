...
if __name__ == '__main__':
    ...
    for ret_var, sp_only in [('dh_ret', 0), ('strad_ret', 0), ('dh_ret', 1), ('strad_ret', 1)]:
        ...
        while (starting + pd.DateOffset(years=13 + counter)) <= pd.to_datetime('20200301', format='%Y%m%d'):
            ...
            for ind, i in enumerate(nf):
                for jnd, j in enumerate(nl):
                    model = XGBRFRegressor(n_estimators=300, max_depth=j, colsample_bynode=i, tree_method='hist',
                                           reg_alpha=0, reg_lambda=0, random_state=counter+1000)
                    model.fit(X_train, Y_train_dm)
                    val_mse[ind, jnd] = mean_squared_error(Y_val, model.predict(X_val) + Y_mean)

            best_i, best_j = divmod(val_mse.argmin(), val_mse.shape[1])
            model = XGBRFRegressor(n_estimators=300, max_depth=nl[best_j], colsample_bynode=nf[best_i], tree_method='hist',
                                   reg_alpha=0, reg_lambda=0, random_state=counter+1000)
            model.fit(X_train, Y_train_dm)
            x_pred = model.predict(X_test) + Y_mean
            reg_pred['rf'] = x_pred
            ...
            for ind, i in enumerate(lambdas):
                for jnd, j in enumerate(nl):
                    model = XGBRegressor(n_estimators=n_trees, max_depth=j, learning_rate=(10 ** i), tree_method='hist',
                                         reg_alpha=0, reg_lambda=0, random_state=counter+1000)
                    callbacks = [xgb.callback.EarlyStopping(rounds=15, save_best=True)]
                    model.fit(X_train, Y_train_dm, eval_set=[(X_val, Y_val_dm)], callbacks=callbacks, verbose=False)
                    val_mse[ind, jnd] = mean_squared_error(Y_val, model.predict(X_val) + Y_mean)

            best_i, best_j = divmod(val_mse.argmin(), val_mse.shape[1])
            model = XGBRegressor(n_estimators=n_trees, max_depth=nl[best_j], learning_rate=(10 ** lambdas[best_i]), tree_method='hist',
                                 reg_alpha=0, reg_lambda=0, random_state=counter+1000)
            callbacks = [xgb.callback.EarlyStopping(rounds=15, save_best=True)]
            model.fit(X_train, Y_train_dm, eval_set=[(X_val, Y_val_dm)], callbacks=callbacks, verbose=False)
            x_pred = model.predict(X_test) + Y_mean
        ...
        out_path = work_dir + 'predicted/tree_all_' + ret_var + '_' + str(sp_only) + '.csv'
        print(out_path)
        pred_out.to_csv(out_path)

        # print the OOS R2
        yreal = pred_out[ret_var].values
        for model_name in ['ols', 'rf', 'gbrt']:
