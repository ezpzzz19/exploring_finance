...
def ae_learning_loop(
    ...
    for i in range(ensemble):
        ...
        test_df = pd.DataFrame(test_dataset[0], columns=char_list)
        ...
        assert torch.all(test_dataset[0] == Z_test), "Contact Chengyu for HELP"
...
if __name__ == "__main__":
    ...
    file_path = "./factors_char_list.csv"
    ...
    for date, monthly_raw in monthly:
        ...
        adj_raw = adj_raw._append(group, ignore_index=True)
        gc.collect()
    ...
    for date, group in monthly_adj:
        ...
        data = data._append(expand, ignore_index=True)
        gc.collect()
    ...
    while (starting + pd.DateOffset(years=8 + counter)) <= pd.to_datetime(
        ...
        pred_out = pred_out._append(pred_batch, ignore_index=True)
