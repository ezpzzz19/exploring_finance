...
try:
    ...
    _HAS_GPU = True
    print("CuPy detected – using GPU for linear algebra.")
except ImportError:
    import numpy as cp          # fallback: cupy → numpy
    import scipy.linalg as csla
    _HAS_GPU = False
    print("CuPy not found – falling back to NumPy/SciPy (CPU).")

import scipy.linalg as sla          # always available for SVD initial guess
import scipy.sparse.linalg as ssla  # always available for sparse SVD

# ── helpers ──────────────────────────────────────────────────────────────────
def _to_gpu(x):
    """Send a numpy array (or value) to the compute device."""
    return cp.asarray(x) if _HAS_GPU else np.asarray(x)

def _to_cpu(x):
    """Bring a cupy array back to numpy (no-op when already numpy)."""
    return cp.asnumpy(x) if _HAS_GPU else np.asarray(x)

# matrix left/right division (following MATLAB function naming) — on GPU
def _mldivide(denom, numer):
    d, n = _to_gpu(np.array(denom, dtype=np.float64)), _to_gpu(np.array(numer, dtype=np.float64))
    return _to_cpu(csla.lstsq(d, n)[0])

def _mrdivide(numer, denom):
    d, n = _to_gpu(np.array(denom, dtype=np.float64).T), _to_gpu(np.array(numer, dtype=np.float64).T)
    return _to_cpu(csla.lstsq(d, n)[0]).T
...
class IPCA_v0(object):
    ...
    def _ipca_als_estimation(self, Gamma0):
        ...
        * Imposes identification assumption on Gamma1 and fFac1:
        ...
        # 2. estimate gamma  (GPU-accelerated kron loop)
        vec_len = self.L * (self.K + self.M)
        g_numer = _to_gpu(np.zeros(vec_len))
        g_denom = _to_gpu(np.zeros((vec_len, vec_len)))
        for t in self.times:
            Fac = pd.concat([fFac1[t], self.gFac[t]])
            fac_arr = _to_gpu(np.array(Fac.values, dtype=np.float64))
            x_arr   = _to_gpu(np.array(self.X[t].values, dtype=np.float64))
            w_arr   = _to_gpu(np.array(self.W[t].values, dtype=np.float64))
            FacOutProd = cp.outer(fac_arr, fac_arr)
            g_numer += cp.kron(x_arr, fac_arr) * self.N_valid[t]
            g_denom += cp.kron(w_arr, FacOutProd) * self.N_valid[t]
        numer_cpu = _to_cpu(g_numer)
        denom_cpu = _to_cpu(g_denom)
        Gamma1_tmp = np.reshape(_mldivide(denom_cpu, numer_cpu), (self.L, self.K + self.M))
        Gamma1 = pd.DataFrame(
        ...
        if self.has_latent:  # GammaBeta orthonormal and fFac1 orthogonal
            ...
            R1 = _to_cpu(csla.cholesky(_to_gpu(GammaBeta1.T.dot(GammaBeta1).values.astype(np.float64))))
...
class IPCA_v1(object):
    ...
    def _linear_als_estimation(
        ...
        numer_g = _to_gpu(np.zeros(self.L * KM))
        denom_g = _to_gpu(np.zeros((self.L * KM, self.L * KM)))
        # dataframe-based  (GPU-accelerated kron loop)
        ct = 0
        for t in Dates:
            x_g = _to_gpu(self.X[t].values.astype(np.float64))
            w_g = _to_gpu(self.W.loc[t].values.astype(np.float64))
            f_g = _to_gpu(Factor[:, ct].astype(np.float64))
            numer_g += cp.kron(x_g, f_g) * self.Nts[t]
            denom_g += cp.kron(w_g, cp.outer(f_g, f_g)) * self.Nts[t]
            ct += 1
        # # ndarray-based
        # ct=0
        # for t in Dates:
        ...
        Gamma1 = np.reshape(_mldivide(_to_cpu(denom_g), _to_cpu(numer_g)), (self.L, KM))
        # 3. normalization
        if K > 0:
    ...
    def _normalization_choice(
        ...
        if normalization_choice == "PCA_positivemean":
            ...
            # GPU-accelerated cholesky + svd
            gf_g = _to_gpu(GammaF.astype(np.float64))
            ff_g = _to_gpu(FactorF.astype(np.float64))
            R1 = _to_cpu(csla.cholesky(_to_gpu(gf_g.T.dot(gf_g))))
            R2_mat = R1.dot(FactorF).dot(FactorF.T).dot(R1.T)
            R2, _, _ = sla.svd(R2_mat)   # keep on CPU – small matrix
